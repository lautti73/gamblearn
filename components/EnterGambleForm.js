...
export const EnterGambleForm = ({ gambleAddress, firstTeam, secondTeam, completed }) => {
    ...
    useEffect(() => {
        ...
        const validate = ( values ) => {
            ...
            if ( values.team != firstTeam && values.team != secondTeam ) {
        ...
        if(isValidated) {
            ...
            if( accounts.length > 0 && ethereum.isConnected() ) {
                ...
                try {
                    ...
                    await gamble.methods.enterGamble( formData.team ).send({
                         from: accounts[0], 
    ...
    return (
        ...
        <form className='my-6 flex flex-col items-center' onSubmit={ handleSubmit } >
            ...
            { formErrors.team && <p className='text-red-400 text-sm font-medium mb-3'>{ formErrors.team }</p> }
            { formErrors.amount && <p className='text-red-400 text-sm font-medium mb-3'>{ formErrors.amount }</p> }
